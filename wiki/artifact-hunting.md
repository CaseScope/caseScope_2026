...
- `mft`
- `usn`
- `ntfs_metadata`, `ntfs_logfile`, `ntfs_log_tracker_export`, `ntfs_logfile_event`
- These use UTC timestamp behavior.
- Best for file creation, modification, deletion, rename, path activity, and filesystem timeline reconstruction.

NTFS `$LogFile` files always create a parent `ntfs_logfile` metadata row. If an optional backend is configured, CaseScope reads backend CSV or SQLite output and emits normalized `ntfs_logfile_event` child rows for file create, delete, rename, move, resident write, non-resident write, directory timestamp update, and directory index update activity. CaseScope can also ingest standalone NTFS Log Tracker CSV or SQLite exports as `ntfs_log_tracker_export` files; those exports use the same child `ntfs_logfile_event` rows and are useful when NTFS Log Tracker is run on a separate Windows analysis workstation. Parent rows preserve parser status, backend availability or export source, companion artifact state, decoded/skipped counts, and fallback warnings. Child rows preserve provenance fields such as `source_parser`, `source_artifact_type`, backend record ID, MFT reference, parent MFT reference, transaction reference, confidence, companion `$MFT` and `$UsnJrnl:$J` state, and parser status. The included NTFSparse wrapper produces transaction-derived, path-partial events; richer NTFS Log Tracker-style exports can provide resolved paths and MFT references. Path-partial rows remain huntable by event type, raw operation, confidence, backend record ID, and transaction reference even when no resolved file path is available. The event detail modal includes an NTFS `$LogFile` Decode section with backend status, enrichment state, MFT references, transaction reference, raw operation, confidence, and parser notes. Raw `$LogFile` binary content is not added to `search_blob`.
