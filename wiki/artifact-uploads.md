...
NTFS `$LogFile` uploads are indexed with a parent `ntfs_logfile` metadata record. If `NTFS_LOG_TRACKER_CMD` is configured for the worker environment, CaseScope runs the configured backend, reads CSV or SQLite output, and emits normalized `ntfs_logfile_event` child rows in the File System tab. Companion `$MFT` and `$UsnJrnl:$J` files are detected when present near the staged artifact and recorded as enrichment state. The included `bin/ntfs_logfile_ntfsparse_adapter.py` wrapper can use an installed `NTFSparse/ntfs_parse` checkout for Linux transaction extraction, but its rows may be path-partial. For richer resolved paths, analysts can run NTFS Log Tracker externally against `$LogFile` and `$MFT`, then upload the exported CSV or SQLite file; CaseScope stores a `ntfs_log_tracker_export` parent row and emits the same normalized `ntfs_logfile_event` child rows. Configure the backend before ingesting `$LogFile` artifacts when possible; files ingested before backend configuration can be reprocessed to replace metadata-only rows with decoded child events. If the backend is unavailable or fails, the upload remains searchable as metadata-only instead of failing ingestion.
