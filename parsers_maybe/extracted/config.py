...
class Config:
    ...
    CELERY_BROKER_URL = os.environ.get('CELERY_BROKER_URL') or f'redis://{REDIS_HOST}:{REDIS_PORT}/{REDIS_DB}'
    CELERY_RESULT_BACKEND = os.environ.get('CELERY_RESULT_BACKEND') or f'redis://{REDIS_HOST}:{REDIS_PORT}/{REDIS_DB}'
    
    # Hayabusa Configuration
    HAYABUSA_BIN = os.environ.get('HAYABUSA_BIN') or os.path.join(BASE_DIR, 'bin', 'hayabusa')
    HAYABUSA_RULES = os.environ.get('HAYABUSA_RULES') or os.path.join(BASE_DIR, 'rules', 'hayabusa-rules')
