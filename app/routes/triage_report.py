...
2. Search ALL events in the case for matches
...
def build_opensearch_query(iocs: Dict, exclusions: List[str] = None) -> Dict:
    """
    Build OpenSearch query to find events matching any extracted IOC.
    Uses should clauses for OR matching across all IOC types.
    """
    should_clauses = []
    ...
    if exclusions:
        ...
        for excl in exclusions:
            must_not.append({"match_phrase": {"search_blob": excl}})
...
def process_triage_report(case_id):
    ...
    def generate():
        ...
        try:
            ...
            # Step 4: Search events and tag matches
            yield f"data: {json.dumps({'stage': 'searching', 'message': 'Searching events...'})}\n\n"
            
            # Build query
            query_body = build_opensearch_query(iocs)
            if not query_body:
            ...
            yield f"data: {json.dumps({'stage': 'found_matches', 'message': f'Found {total_matches} matching events', 'total_matches': total_matches})}\n\n"
            ...
            yield f"data: {json.dumps({'stage': 'tagging', 'message': 'Tagging matching events...', 'processed': 0, 'total': total_matches})}\n\n"
            ...
            try:
                ...
                while hits:
                    ...
                    yield f"data: {json.dumps({'stage': 'tagging', 'message': f'Tagged {tagged_count} events...', 'processed': processed_count, 'total': total_matches, 'tagged': tagged_count})}\n\n"
                ...
                logger.error(f"[TRIAGE] Scroll search failed: {e}")
                yield f"data: {json.dumps({'stage': 'error', 'message': f'Search error: {str(e)}'})}\n\n"
                return
            
            # Final summary
            yield f"data: {json.dumps({'stage': 'complete', 'message': f'Triage complete! Tagged {tagged_count} events.', 'tagged': tagged_count, 'skipped': skipped_count, 'iocs_added': iocs_added, 'systems_added': systems_added, 'total_matches': total_matches})}\n\n"
