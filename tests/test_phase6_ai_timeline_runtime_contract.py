...
class Phase6AITimelineRuntimeContractTestCase(unittest.TestCase):
    def test_timeline_generator_uses_shared_router_and_surfaces_runtime(self):
        with open(
            os.path.join(REPO_ROOT, 'utils', 'ai_timeline_generator.py'),
            'r',
            encoding='utf-8',
        ) as handle:
            source = handle.read()
