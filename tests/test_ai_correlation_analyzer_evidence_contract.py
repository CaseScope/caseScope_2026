...
class AICorrelationAnalyzerEvidenceContractTestCase(unittest.TestCase):
    ...
    def test_prompt_includes_context_json_ids_and_rules(self):
        ...
        self.assertIn('context:known_good or context:noise with status unknown does not prove', analyzer.captured_prompt)
        ...
        self.assertIn('Do not say "known-good," "noise," "noise rule,"', analyzer.captured_prompt)
        self.assertIn('unless a referenced known noise, known_good, or user_role context fact supports it', analyzer.captured_prompt)
