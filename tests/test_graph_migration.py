...
class GraphMigrationContractTestCase(unittest.TestCase):
    ...
    def test_projection_state_migration_uses_durable_case_scoped_table(self):
        ...
        self.assertIn("current_window_start_utc", columns)
        self.assertIn("current_window_end_utc", columns)
        self.assertIn("last_completed_window_end_utc", columns)
        self.assertIn("windows_completed", columns)
