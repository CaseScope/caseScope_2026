...
class Phase7PatternSuppressionStageTestCase(unittest.TestCase):
    ...
    def test_pattern_callers_use_shared_suppression_helper(self):
        ...
        self.assertIn("apply_pattern_suppression,", case_analyzer_source)
        self.assertIn("suppression_result = apply_pattern_suppression(", case_analyzer_source)
        self.assertNotIn("suppression_matches = get_pattern_suppression_matches(", case_analyzer_source)

        self.assertIn("apply_pattern_suppression,", rag_tasks_source)
        self.assertIn("suppression_result = apply_pattern_suppression(", rag_tasks_source)
