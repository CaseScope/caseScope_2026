...
- The async task surface no longer has the original shared failure-state or cancellation drift at the reviewed boundaries: archive/memory/PCAP/case-analysis stop points now use cooperative cancellation checks, and the remaining async route-contract work is the inconsistent status-envelope shape across blueprints rather than task termination semantics.
