...
- `utils/deterministic_evidence_engine.py` and `utils/candidate_extractor.py` are live review surfaces, not hypothetical internals: post-Review 11 implementation closed the deterministic UTC query-column drift, the sequence-chain ordering false-positive risk, and the malformed-anchor wall-clock fallback here, and follow-up work has now added explicit sequence evaluability metadata, while sequence scoring-policy integration and extractor SQL hardening still remain open.
