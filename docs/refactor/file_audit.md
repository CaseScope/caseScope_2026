...
| `tasks/rag_tasks.py` | yes | 3029 | AI correlation task now reuses shared pattern-analysis helpers for setup, per-pattern progress payload construction, per-pattern iteration orchestration, cleanup handling, final completion payload construction, final overlap annotation, and response packaging, while still retaining task-specific provider initialization and outer error/stat aggregation. |
| `pipeline/__init__.py` | yes | 65 | Shared pipeline package export surface now populated beyond the original pattern-analysis wrappers. |
| `pipeline/pattern_analysis.py` | yes | 699 | Pattern-analysis stage surface now includes shared extractor/evidence setup wrappers, task-side progress payload construction, task-side completion payload construction, task-side iteration orchestration, task-side cleanup handling, task-side AI execution orchestration, final overlap annotation, task response packaging, concise per-pattern threat-intel prompt assembly, census, eligibility filtering, stable ordering, evidence-package selection, suppression, materialization, package processing, evaluation, persistence, and rule-based evaluation helpers. |
