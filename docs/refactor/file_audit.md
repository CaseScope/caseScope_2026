...
| `utils/deterministic_evidence_engine.py` | yes | 2190 | Deterministic evaluation core; post-Review 11 implementation now executes coverage, runtime-normalized query templates, burst, sequence, and spread SQL against the UTC-normalized event-time column, and sequence-chain validation now walks stepwise off the previously matched event with active correlation-field scoping while sequence evaluability and deterministic malformed-anchor fallback work remain open here. |
