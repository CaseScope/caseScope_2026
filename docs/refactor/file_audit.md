...
| `tasks/rag_tasks.py` | yes | 3028 | AI correlation task reuses shared pattern-analysis helpers and no longer feeds OpenCTI ATT&CK context into the persisted AI scoring path; the remaining async drift on this surface is now cooperative cancellation on case-analysis/PCAP flows after the long-running failure-state contract was tightened across the reviewed `tasks/celery_tasks.py` and `tasks/rag_tasks.py` entrypoints. |
...
- The async task surface no longer has the original shared failure-state drift: both the job-backed archive/memory/PCAP tasks and the reviewed long-running/result-cached entrypoints in `tasks/celery_tasks.py` / `tasks/rag_tasks.py` now raise after writing failed progress or durable state. The remaining async contract gap is cooperative cancellation, which is still open on the PCAP and case-analysis surfaces after archive and memory gained stage-aware stop checkpoints.
