...
| `utils/deterministic_evidence_engine.py` | yes | 2063 | Deterministic evaluation core; post-Review 11 implementation now executes coverage, runtime-normalized query templates, burst, sequence, and spread SQL against the UTC-normalized event-time column while sequence evaluability/ordering fixes and deterministic malformed-anchor fallback work remain open here. |
