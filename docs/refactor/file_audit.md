...
| `tasks/rag_tasks.py` | yes | 3028 | AI correlation task reuses shared pattern-analysis helpers and no longer feeds OpenCTI ATT&CK context into the persisted AI scoring path; the remaining async failure/cancellation drift is now narrowed to the non-job long-running/result-cache surfaces here and in `tasks/celery_tasks.py`, plus the still-open cooperative-cancellation gap on case-analysis and PCAP flows after archive and memory gained stage-aware stop checkpoints. |
| `routes/rag.py` | yes | 2678 | RAG route surface; Ask-AI prompt/response handling labels TI and semantic context as grounded assistance only, and the legacy unified-findings compatibility wrapper has now been removed after live callers migrated to the canonical findings blueprint. |
...
- `utils/ioc_extractor.py` remains the canonical IOC orchestration facade. The public helper surface now includes `run_deterministic_ioc_extraction()`, and audit mode now preserves the deterministic extraction plus accepted audit overlay metadata alongside the audited IOC view so downstream consumers can distinguish detector output from AI corrections.
...
- `utils/deterministic_evidence_engine.py` and `utils/candidate_extractor.py` are live review surfaces, not hypothetical internals: post-Review 11 implementation closed the deterministic UTC query-column drift, the sequence-chain ordering false-positive risk, the malformed-anchor wall-clock fallback, and the sequence-specific telemetry/excluded-weight gap here, while extractor SQL hardening and rarest-anchor follow-up still remain open.
...
- Shared case access on `/api/...` routes now flows through a JSON-aware forbidden handler, so future plan text should no longer describe HTML 403 responses as the remaining live issue at that boundary; the still-open slice is the inconsistent async status envelope shape across multiple blueprints.
...
- The L1 chat boundary no longer has permissive tool-argument validation: `utils/chat_agent.py` now validates decoded tool arguments against `TOOL_DEFINITIONS` before permission checks or execution. `utils/chat/dispatch.py` still has provenance fallback behavior on the L1 boundary, so Phase 6 should not yet be described as strict-provenance complete.
- The async task surface still has shared failure-state and cancellation contract drift, but both gaps are now narrower than the original Review 11 description: job-backed archive, memory, and PCAP tasks raise after durable failure writes; archive and memory now also have cooperative cancellation checkpoints; `tasks/celery_tasks.py`, `tasks/rag_tasks.py`, and the PCAP/case-analysis cancellation surfaces still need the same treatment end to end.
