...
| `tasks/rag_tasks.py` | yes | 3038 | AI correlation task now reuses shared pattern-analysis helpers for setup, per-pattern extraction shaping, per-pattern AI execution, and final overlap annotation, while still retaining task-specific progress reporting, provider initialization, cleanup handling, and error/stat aggregation. |
| `pipeline/__init__.py` | yes | 55 | Shared pipeline package export surface now populated beyond the original pattern-analysis wrappers. |
| `pipeline/pattern_analysis.py` | yes | 556 | Pattern-analysis stage surface now includes shared extractor/evidence setup wrappers, task-side extraction shaping, task-side AI execution orchestration, final overlap annotation, concise per-pattern threat-intel prompt assembly, census, eligibility filtering, stable ordering, evidence-package selection, suppression, materialization, package processing, evaluation, persistence, and rule-based evaluation helpers. |
