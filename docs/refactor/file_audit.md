...
| `utils/case_analyzer.py` | yes | 1203 | Current orchestration bottleneck; gap detection now routes through `pipeline.detect_anomalies`, pattern analysis through shared `pipeline.pattern_analysis` helpers, and TI enrichment through `pipeline.case_enrichment` rather than a direct overlay call site. |
