...
| `utils/chat_tools.py` | yes | present | Direct chat-tool producer surface; `query_events`, `count_events`, and `get_findings` now emit shared payload provenance metadata instead of falling back to dispatcher defaults, leaving only the IOC and threat-intel direct producers to finish the Phase 6.5 bypass audit. |
