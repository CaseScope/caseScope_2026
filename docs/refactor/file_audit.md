...
- The async task surface still has shared failure-state and cancellation contract drift, but the remaining failure-state gap is now narrower than the original Review 11 description: job-backed archive, memory, and PCAP tasks raise after durable failure writes, while `tasks/celery_tasks.py` and `tasks/rag_tasks.py` still need the same broker-truthful contract on their long-running/result-cached flows.
