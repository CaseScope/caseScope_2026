...
- `utils/ioc_extractor.py` still contains mixed regex, AI normalization, merge, and import-pipeline concerns internally, but Phase 9 has now promoted it further with an explicit public export contract for the live boundary while leaving the internal decomposition work for later slices.
