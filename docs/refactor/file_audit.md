...
| `utils/deterministic_evidence_engine.py` | yes | 2238 | Deterministic evaluation core; post-Review 11 implementation now executes coverage, runtime-normalized query templates, burst, sequence, and spread SQL against the UTC-normalized event-time column, sequence-chain validation now walks stepwise off the previously matched event with active correlation-field scoping, and malformed anchor timestamps now take an explicit deterministic unknown-window path while the broader sequence evaluability contract still remains open here. |
