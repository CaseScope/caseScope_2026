...
| `tasks/rag_tasks.py` | yes | 3025 | AI correlation task now reuses shared pattern-analysis helpers for setup, per-pattern progress payload construction, per-pattern iteration orchestration, final overlap annotation, and response packaging, while still retaining task-specific provider initialization, cleanup handling, and outer error/stat aggregation. |
| `pipeline/__init__.py` | yes | 61 | Shared pipeline package export surface now populated beyond the original pattern-analysis wrappers. |
| `pipeline/pattern_analysis.py` | yes | 676 | Pattern-analysis stage surface now includes shared extractor/evidence setup wrappers, task-side progress payload construction, task-side iteration orchestration, task-side AI execution orchestration, final overlap annotation, task response packaging, concise per-pattern threat-intel prompt assembly, census, eligibility filtering, stable ordering, evidence-package selection, suppression, materialization, package processing, evaluation, persistence, and rule-based evaluation helpers. |
