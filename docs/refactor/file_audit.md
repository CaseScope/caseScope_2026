...
- `parsers/memory_parser.py`, `models/memory_data.py`, and the direct memory-route serializers now emit shared parser and field provenance metadata instead of leaving that entirely to later chat/runtime decoration, but the memory family still persists into dedicated `memory_*` tables rather than the shared `ParsedEvent` contract and therefore remains a documented partial exception.
