...
| `utils/unified_findings.py` | yes | 460 | Unified finding read path area; legacy fallback readers have been retired so the mirrored ClickHouse store is now the only canonical read path. |
...
| `utils/pattern_check_definitions.py` | yes | 3046 | Duplicate `security_tool_tampering` entry removed; deterministic pattern registry now has a single authoritative definition for that pattern. |
| `utils/pattern_event_mappings.py` | yes | 1618 | Live companion file for pattern semantics and mappings. |
| `utils/hayabusa_correlator.py` | yes | 745 | Already emits unified findings through `build_hayabusa_correlation_finding`; remaining work is contract verification and downstream parity cleanup, not first-time migration. |
...
- `utils/unified_findings.py` now reads only from the mirrored ClickHouse findings store, so any remaining parity gaps must be fixed in the producer sync path rather than by reviving legacy readers.
