...
| `utils/case_analyzer.py` | yes | 1537 | Current orchestration bottleneck and overlay call site; gap detection and more of the pattern-analysis orchestration now route through shared `pipeline/` stage helpers. |
| `pipeline/__init__.py` | yes | 35 | Shared pipeline package export surface now populated beyond the original pattern-analysis wrappers. |
| `pipeline/pattern_analysis.py` | yes | 138 | Pattern-analysis stage surface now includes shared census, eligibility filtering, stable ordering, and evidence-package selection helpers in addition to the existing extractor/evidence wrappers. |
