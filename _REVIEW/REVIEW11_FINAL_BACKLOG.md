...
5. `GAP-V2-SEQUENCE-COVERAGE` — `GAP` / `HIGH`
   Reviews 1, 3a, and 3b all found that sequence contribution is still always treated as evaluable once a sequence config exists; Scoring 2.0 does not yet express "not evaluable because telemetry is missing" for sequences. This makes package scoring and excluded-weight reporting wrong for sequence-dependent patterns under partial telemetry. Post-Review 11 follow-up on 2026-04-20 partially progressed the item by adding explicit sequence evaluability metadata to `SequenceResult` and sequence producer inputs, plus focused regression coverage that distinguishes missing-telemetry characterization from anchor-window-unavailable cases without changing current score/excluded-weight behavior yet. The item remains open because sequence definitions still do not carry sequence-specific source requirements, so the engine cannot truthfully decide when a missing chain step should be excluded versus treated as a real miss. Proposed fix: add sequence-level telemetry requirements, then wire sequence evaluability into Scoring 2.0 excluded-weight handling. Suggested test-first coverage: characterization coverage is now present; a scoring-behavior test should land with the eventual weight-policy change.
...
- `GAP-V2-SEQUENCE-COVERAGE` characterization fixtures are now in place for missing-telemetry versus anchor-window-unavailable sequence evaluability metadata; the next test addition should accompany the eventual excluded-weight/score-policy change once sequence-specific telemetry requirements exist.
