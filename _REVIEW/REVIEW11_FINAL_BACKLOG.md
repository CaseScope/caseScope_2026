...
12. `DRIFT-JSON-403-ERROR-SHAPE` — `DRIFT` / `MEDIUM`
    Reviews 7a, 7b, and 10 confirmed that otherwise-JSON APIs can still emit HTML 403 responses because shared case access enforcement uses `abort(403)`, and async status payloads still vary by blueprint. Post-Review 11 follow-up on 2026-04-20 partially progressed this by adding one shared API forbidden-error handler and updating the shared case-access abort path to emit an explicit `Access denied` description, so JSON routes now return the canonical `{"success": false, "error": ...}` 403 shape at the shared boundary. The item remains open because async status endpoints in `routes/iocs.py`, `routes/rag.py`, `routes/parsing.py`, and `routes/hunting.py` still expose divergent payload contracts. Proposed fix for the remaining slice: route those status responses through one shared async-status envelope helper. Suggested test-first coverage: yes.
