...
- Operational follow-up on 2026-04-21 started the ClickHouse mutation-remediation plan with the least risky slice: analyst event state now writes to a dedicated `event_analyst_state` overlay table and the main analyst-tagged read surfaces consume that overlay instead of mutating `events`. The larger noise and IOC reset/tagging mutation bundles remain open.
