...
23. `TEST-SOURCE-TEXT-GUARDRAILS` — `TEST` / `MEDIUM`
    Review 10 found several regression suites that still assert source text, helper names, or decorator stacking instead of behavior. Follow-up on 2026-04-21 replaced the named Phase 7 `tasks/rag_tasks.py` helper-delegation source-inspection tests with behavioral `ai_pattern_correlation(...)` coverage, but broader source-text/helper-name assertions still remain elsewhere in the refactor-era suites. This does not currently produce wrong runtime output, but it still makes safe cleanup needlessly expensive until the remaining tests are converted.
