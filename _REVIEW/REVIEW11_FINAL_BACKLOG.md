...
- `PERF-CLICKHOUSE-DELETE-MUTATION-CONTRACT` — `PERF` / `HIGH`
  The remaining high-risk ClickHouse workflows are destructive deletes and dedup deletes. Follow-up work made the contract materially safer by waiting for case rebuild and single-file delete mutations where subsequent logic depends on completion, by making the case-wide delete task report mutation submission instead of implying the rows are already gone, and by making automatic post-ingest dedup skip very large per-artifact rewrites instead of unconditionally issuing them. The unresolved risk is that explicit destructive workflows still rely on large `ALTER TABLE ... DELETE` rewrites under production load, especially case-wide deletes, permanent case deletion, and manual dedup on artifact families that remain too large for the automatic safety ceiling.
