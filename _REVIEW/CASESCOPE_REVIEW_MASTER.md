...
| `DRIFT-OPENCTI-CONNECTOR-CACHE-FRESHNESS` | Review 9 found that `utils/opencti.py` memoizes the active connector catalog for the lifetime of the process with no TTL or invalidation path, so admin/status and IOC-hint surfaces can drift from live connector state until restart. | Review 9 | Review 10 |
| `GAP-THREAT-INTEL-CONTEXT-CACHING` | Review 9 found that `utils/threat_intel_context.py` performs uncached per-IOC provider lookups during prompt construction and still routes CVE values through exact IOC lookup before separate vulnerability-context fetches, creating avoidable external fanout on narrative/report surfaces. | Review 9 | Review 10 |
