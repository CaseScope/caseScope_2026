...
| `GAP-SCORE-DISPLAY-CONTRACT` | Review 7b confirmed that live unified-findings route/presentation surfaces still converge on normalized `confidence` / `severity` display only; the Scoring 2.0 compact analyst/LLM score display contract is still not implemented as one shared response/presentation surface. | Review 1, Review 7b | Review 10 |
...
| `RISK-L1-TOOL-SCHEMA-VALIDATION` | Review 6 found that model-supplied tool arguments are JSON-decoded and passed to Python callsites without strict request-shape validation, so unknown keys and type-mismatched payloads can fail open instead of being rejected as invalid structured tool calls. Review 11 consolidated this as a premium chat/runtime release-risk item. | Review 6, Review 11 | Review 11 |
| `DRIFT-CASE-ANALYZER-FINALIZE` | Review 2b found that `utils/case_analyzer.py` still owned terminal persistence, summary shaping, progress bookkeeping, and unified-findings sync. Review 8 re-verified that this still survives in live code; Review 8 only landed degraded-phase bookkeeping fixes, so the "orchestration-only" claim remains only partially closed. | Review 2b, Review 8 | Review 10 |
...
| `DRIFT-LEGACY-UNIFIED-FINDINGS-ROUTE` | Review 7b verified that `routes/findings.py` is registered and canonical, but live hunting templates still call `/api/rag/unified-findings/<case_id>`. The legacy RAG route now delegates to the canonical serializer, but caller migration / route retirement is still pending. | Review 7b | Review 10 |
...
| `RISK-VIEWER-WRITE-POLICY-DRIFT` | Review 7a found, and Review 11 re-verified, that several mutating or task-triggering case routes (`routes/analysis.py`, `routes/iocs.py`, `routes/known_users.py`, `routes/known_systems.py`, `routes/case_files.py`) still rely on login plus case lookup without a shared viewer-write guard, so read-only users can still start analyses or mutate case-side state. | Review 7a, Review 11 | Review 11 |
| `GAP-TASK-FAILURE-STATE-CONTRACT` | Review 8 found that multiple background tasks persist failure details locally and then return `{"success": false, ...}` instead of raising, so Celery records operational failures as `SUCCESS` and broker-level retry/dead-letter/monitoring semantics are not authoritative. | Review 8 | Review 10 |
| `GAP-ASYNC-CANCELLATION-CONTRACT` | Review 8 found that long-running case/memory/archive/PCAP task and pipeline flows expose progress updates but no shared cooperative cancellation checks, so revoke/abort behavior depends on process interruption or time limits rather than stage-aware stop points. | Review 8 | Review 10 |
