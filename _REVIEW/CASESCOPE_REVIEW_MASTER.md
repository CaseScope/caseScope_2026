...
| `DRIFT-JSON-403-ERROR-SHAPE` | Review 7a found that route code mostly returns JSON `{"success": false, "error": ...}`, but shared case access enforcement in `models/case.py` still uses `abort(403)`, so otherwise-JSON APIs can emit HTML 403s and async status payloads still vary by blueprint. Post-Review 11 follow-up has now normalized API-side 403s through one shared forbidden-error handler and an explicit access-denied abort description, but the async task status envelope drift across `routes/iocs.py`, `routes/rag.py`, `routes/parsing.py`, and `routes/hunting.py` remains open. | Review 7a | Review 10 |
...
| `GAP-TASK-FAILURE-STATE-CONTRACT` | Review 8 found that multiple background tasks persist failure details locally and then return `{"success": false, ...}` instead of raising, so Celery records operational failures as `SUCCESS` and broker-level retry/dead-letter/monitoring semantics are not authoritative. Post-Review 11 follow-up has now fixed the job-backed archive, memory, and PCAP task surfaces to raise after durable failure writes, but `tasks/celery_tasks.py` and `tasks/rag_tasks.py` still contain the remaining open async failure-state drift. | Review 8 | Review 10 |
| `GAP-ASYNC-CANCELLATION-CONTRACT` | Review 8 found that long-running case/memory/archive/PCAP task and pipeline flows expose progress updates but no shared cooperative cancellation checks, so revoke/abort behavior depends on process interruption or time limits rather than stage-aware stop points. Post-Review 11 follow-up has now added cooperative cancellation checkpoints to the archive and memory task loops plus an archive cancel route aligned with the existing memory job-status contract, but PCAP and case-analysis flows still lack the same shared stop-point model. | Review 8 | Review 10 |
