...
| `PERF-CLICKHOUSE-EVENT-MUTATION-PRESSURE` | Operational follow-up on 2026-04-21 completed the mutable event-tag remediation slices that fit the overlay pattern cleanly: analyst state, noise state, and IOC event tags now write to dedicated overlay tables and the main hunt/chat/RAG/deterministic read paths project effective state at read time instead of mutating `events`. A later 2026-04-21 follow-up also made the post-ingest dedup path skip very large destructive rewrites and report those skips truthfully, while buffer-flush and rebuild result payloads now stop overstating completion. The remaining open ClickHouse risk is still explicit destructive delete/dedup workflows that rely on heavy `ALTER TABLE ... DELETE` mutations when an operator intentionally runs them. | 2026-04-21 ClickHouse audit | Operational follow-up |
