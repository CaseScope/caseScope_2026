...
| `TEST-SOURCE-TEXT-GUARDRAILS` | Review 10 found that multiple refactor-era regression suites asserted source text, helper names, or decorator stacking instead of behavior, making harmless cleanup fail tests. Post-Review 11 follow-up converted the Phase 5 IOC text/merge contracts, the Phase 7 task/case helper-delegation suites, and the named non-Phase-7 guardrails in `tests/test_phase1_contract_surfaces.py`, `tests/test_phase2_unified_findings_store.py`, `tests/test_phase6_ai_router_contract.py`, `tests/test_opencti_exact_enrichment.py`, and `tests/test_phase15_ti_overlay_separation.py` to runtime behavior; the remaining debt is now concentrated in a smaller residual source/AST set including `tests/test_phase3_route_decomposition.py`, `tests/test_startup_hardening_regressions.py`, `tests/test_phase0_feature_gating.py`, `tests/test_pattern_overlay.py`, `tests/test_ai_adapter_routing.py`, and the prompt-wording/source contract in `tests/test_phase7_pattern_threat_intel_stage.py`. | Review 10 | Review 11 |
