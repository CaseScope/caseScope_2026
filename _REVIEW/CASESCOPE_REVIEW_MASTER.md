...
| `PERF-CLICKHOUSE-EVENT-MUTATION-PRESSURE` | Operational follow-up on 2026-04-21 started the `events` mutation remediation by moving analyst-tag writes and the main analyst-tagged read surfaces onto the `event_analyst_state` overlay table, but manual noise tagging plus IOC/noise reset workflows still mutate `events` directly and remain the larger open pressure source. | 2026-04-21 ClickHouse audit | Operational follow-up |
