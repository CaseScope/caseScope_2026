...
| `TEST-SOURCE-TEXT-GUARDRAILS` | Review 10 found that multiple refactor-era regression suites asserted source text, helper names, or decorator stacking instead of behavior, making harmless cleanup fail tests. Post-Review 11 follow-up converted the Phase 7 task/case helper-delegation suites and the Phase 5 IOC text/merge contracts to runtime behavior; the remaining debt is now concentrated in a smaller non-Phase-7 set such as `tests/test_phase1_contract_surfaces.py`, `tests/test_phase2_unified_findings_store.py`, `tests/test_phase6_ai_router_contract.py`, and `tests/test_opencti_exact_enrichment.py`. | Review 10 | Review 11 |
