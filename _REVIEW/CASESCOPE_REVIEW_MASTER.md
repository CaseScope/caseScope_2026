...
| `PERF-CLICKHOUSE-EVENT-MUTATION-PRESSURE` | Operational follow-up on 2026-04-21 completed the mutable event-tag remediation slices that fit the overlay pattern cleanly: analyst state, noise state, and IOC event tags now write to dedicated overlay tables and the main hunt/chat/RAG/deterministic read paths project effective state at read time instead of mutating `events`. Later 2026-04-21 follow-up then made the post-ingest dedup path skip very large destructive rewrites and report those skips truthfully, moved manual dedup off the request thread onto explicit async start/status contracts, made permanent case/client deletion plus case-wide PCAP rebuild wait for ClickHouse delete mutations before claiming completion, tightened the remaining PCAP destructive network-log paths so single-PCAP delete and PCAP log reindex also wait for `network_logs` delete mutations before reporting completion or reinserting rows, serialized the remaining explicit case-wide `events` rewrites so delete/rebuild/dedup work no longer overlaps under load, made the remaining single-file / single-PCAP delete and rebuild-reset flows fail closed when the ClickHouse delete fails instead of dropping metadata anyway, and finally made parser-side partial-row cleanup wait for the file-scoped delete mutation before surfacing parse failure. The remaining open ClickHouse risk is now mostly the underlying operational cost of explicit destructive `ALTER TABLE ... DELETE` rewrites, not misleading completion semantics. | 2026-04-21 ClickHouse audit | Operational follow-up |
