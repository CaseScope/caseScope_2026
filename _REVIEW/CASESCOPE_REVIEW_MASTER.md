...
| `GAP-V2-SEQUENCE-COVERAGE` | Scoring 2.0 sequence handling still lacks the final exclude-vs-evaluable scoring behavior under missing telemetry, but post-Review 11 follow-up has now added explicit sequence evaluability metadata and characterization coverage so the remaining work is narrowed to sequence-specific telemetry requirements plus score/excluded-weight integration. | Review 1 | Review 3 |
