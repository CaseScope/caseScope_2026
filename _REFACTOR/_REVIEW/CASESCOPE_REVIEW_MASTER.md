...
| `GAP-BEHAVIORAL-DETECTOR-INTEGRATION` | Resolved in post-Review 11 implementation follow-up: behavioral gap findings are now registered in `GAP_FINDING_CHECK_BINDINGS` for canonical `behavioral_*` patterns/checks, and the deterministic engine consumes those mapped findings directly instead of leaving them on the unmapped producer-input side path. | Review 3b | Review 10 |
...
| `DRIFT-MEMORY-PARSER-PROVENANCE-CONTRACT` | Resolved in post-Review 11 implementation follow-up plus 2026-04-22 verification: the memory family now persists row-level `parser_provenance` on the `memory_*` models, `parsers/memory_parser.py` sets that metadata during ingest, and serializers/result envelopes emit shared parser and field provenance metadata. Memory still uses dedicated `memory_*` tables rather than `ParsedEvent`, but that is now treated as a documented schema distinction rather than an open provenance-loss contract bug. | Review 4b | Review 8 |
...
| `TEST-SOURCE-TEXT-GUARDRAILS` | Resolved in post-Review 11 implementation follow-up: the genuinely brittle helper-name/source regressions across the Phase 5 IOC, Phase 7 task/case orchestration, unified-findings mirroring, IOC router, OpenCTI helper-boundary, TI-enrichment delegation, feature-gating, and pattern-overlay suites were converted to runtime behavior checks. The remaining source/AST readers are now intentionally treated as declarative contract tests for static route/startup wiring, prompt wording, or user-visible UI text rather than open helper-name guardrail debt. | Review 10 | Review 11 |
...
| 2026-04-20 | Review 4b verified that the browser / log / vendor event parsers still populate `timestamp_utc` through the base parser contract, while the memory parser remains a separate non-`ParsedEvent` ingest path. | Keeps Review 3's UTC thread scoped to downstream query consumers for event parsers while explicitly preserving the memory-family schema distinction without treating it as an open parser-provenance loss bug once row-level and emitted provenance were verified. | Review 5, Review 8, Review 10 |
...
| 2026-04-22 | Treat the remaining ClickHouse delete-mutation item as a verified operational limitation, not as an unresolved app-level completion-contract bug. | The live delete/rebuild/dedup paths now wait, fail closed, serialize explicit case-wide rewrites, and have focused regression coverage; what remains is mostly ClickHouse mutation cost under load, plus the deployment caveat that rewrite serialization weakens if Redis is unavailable for the guard. | Operational follow-up, `tests/test_clickhouse_delete_dedup_contracts.py`, `utils/clickhouse.py`, `utils/event_deduplication.py`, ClickHouse safety |
...
| 2026-04-21 | Treat IOC import-entry construction as a helper boundary even while import-prep and persistence remain in the extractor facade. | Case-scoped duplicate lookup and type-aware IOC entry assembly are cohesive enough to extract behind wrappers without changing the tested `_create_ioc_entry*` surface, reducing facade sprawl before the larger process/save split. | Operational follow-up, `utils/ioc_import_entries.py`, `utils/ioc_extractor.py` |
| 2026-04-21 | Treat known-system and known-user IOC side effects as helper boundaries even before the broader process/save split. | The per-IOC known-entity lookup decisions are small, test-covered units that can move behind wrappers without changing `process_extraction_for_import(...)`, leaving the remaining facade debt concentrated in the orchestration-heavy import/save functions. | Operational follow-up, `utils/ioc_known_entities.py`, `utils/ioc_extractor.py` |
| 2026-04-22 | Treat IOC import-prep orchestration as its own helper boundary while the extractor facade remains partially open. | `process_extraction_for_import(...)` had become the next orchestration-heavy block after the alias, entry-construction, and known-entity extractions; moving it behind `utils/ioc_import_processing.py` preserves the public wrapper while shrinking the facade down to the remaining regex extraction and persistence/save paths. | Operational follow-up, `utils/ioc_import_processing.py`, `utils/ioc_extractor.py`, IOC extraction regressions |
| 2026-04-22 | Treat IOC save/persistence orchestration as its own helper boundary while the extractor facade remains partially open. | `save_extracted_iocs(...)` had become the last large persistence block after the import-prep extraction; moving it behind `utils/ioc_persistence.py` preserves the public wrapper while reducing the remaining extractor-facade debt to the regex extraction path. | Operational follow-up, `utils/ioc_persistence.py`, `utils/ioc_extractor.py`, IOC extraction regressions |
| 2026-04-22 | Treat regex IOC extraction and derived-indicator harvesting as helper boundaries behind the public IOC facade. | Moving `RegexIOCExtractor` and `extract_derived_indicator_candidates(...)` into `utils/ioc_regex_extractor.py` finishes the facade split while preserving the exported `utils/ioc_extractor.py` surface for routes, tasks, and enrichers. | Operational follow-up, `utils/ioc_regex_extractor.py`, `utils/ioc_extractor.py`, IOC extraction regressions |
