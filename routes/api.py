...
# =============================================================================
# CASE ARCHIVE/RESTORE API
# =============================================================================

@api_bp.route('/case/<case_uuid>/archive', methods=['POST'])
@login_required
def start_case_archive(case_uuid):
    """Start archiving a case
    
    Compresses storage and evidence folders to ZIP files and cleans up originals.
    
    Returns:
        Job ID for tracking progress
    """
    try:
        from models.archive_job import ArchiveJob, ArchiveJobType, ArchiveJobStatus, ARCHIVE_STAGES
        from models.system_settings import SystemSettings, SettingKeys
        from models.audit_log import AuditLog
        ...
        # Check if archive already in progress
...
@api_bp.route('/case/<case_uuid>/archive/status')
@login_required
def get_archive_status(case_uuid):
    ...
    try:
        ...
        # Get most recent archive job
        job = ArchiveJob.query.filter_by(
            case_uuid=case_uuid,
            job_type=ArchiveJobType.ARCHIVE.value
        ).order_by(ArchiveJob.created_at.desc()).first()
        
        if not job:
            return jsonify({'success': False, 'error': 'No archive job found'}), 404
        
        # Get real-time progress from Redis if running
        redis_progress = {}
        ...
        # Merge Redis progress for real-time updates
        if redis_progress:
...
@api_bp.route('/case/<case_uuid>/archive/info')
@login_required
def get_archive_info(case_uuid):
    """Get archive information for an archived case
    
    Returns manifest data if case is archived.
    """
    try:
        from models.system_settings import SystemSettings, SettingKeys
...
@api_bp.route('/case/<case_uuid>/restore', methods=['POST'])
@login_required
def start_case_restore(case_uuid):
    """Start restoring an archived case
    
    Extracts ZIP files back to storage and evidence folders.
    
    Request body:
    {
        "delete_archive": false  // Optional: delete archive after restore
    }
    ...
    """
    try:
        from models.archive_job import ArchiveJob, ArchiveJobType, ArchiveJobStatus, RESTORE_STAGES
        from models.system_settings import SystemSettings, SettingKeys
        from models.audit_log import AuditLog
...
@api_bp.route('/case/<case_uuid>/restore/status')
@login_required
...
@api_bp.route('/case/<case_uuid>/storage/size')
@login_required
